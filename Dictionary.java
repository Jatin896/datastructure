...
private HashMap<Character,DictionaryNode> roots = new HashMap<Character,DictionaryNode>();
	...
	public void insert(String string) {
		if (!roots.containsKey(string.charAt(0))) {
			roots.put(string.charAt(0), new DictionaryNode());
		}
	...
	private void insertWord(String string, DictionaryNode node) {
		final DictionaryNode nextChild;
		if (node.children.containsKey(string.charAt(0))) {
			nextChild = node.children.get(string.charAt(0));
		} else {
			nextChild = new DictionaryNode();
	...
	private boolean searchFor(String string, DictionaryNode node) {
		if (string.length()==0) {
			if (node.endOfWord) {
				return true;
			} else {
